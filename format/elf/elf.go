...
// TODO: dyn tags functions
// TODO: section syms/desc
// TODO: dwarf
...
func elfDecodeGNUHash(d *decode.D, ec elfContext, strTab string) {
	...
	// d.FieldArray("values", repeatFn(int(nChain), func(d *decode.D) { d.FieldU32("chain") }))
}
...
func elfDecodeSectionHeader(d *decode.D, ec elfContext, sh sectionHeader) {
	shFlags := func(d *decode.D, archBits int) bool {
		execInstr := false
		d.FieldStruct("flags", func(d *decode.D) {
			if d.Endian == decode.LittleEndian {
	...
	case 32:
		...
		execInstr = shFlags(d, ec.archBits)
		addr = d.FieldU("addr", ec.archBits, scalar.Hex)
		offset = d.FieldU("offset", ec.archBits)
		size = d.FieldU32("size", scalar.Hex)
		d.FieldU32("link")
		d.FieldU32("info")
		d.FieldU32("addralign")
		entSize = d.FieldU32("entsize")
	case 64:
		d.FieldU32("name", strTable(ec.strTabMap[STRTAB_SHSTRTAB]))
		typ = d.FieldU32("type", sectionHeaderTypeMap, scalar.Hex)
		execInstr = shFlags(d, ec.archBits)
	...
	case SHT_GNU_HASH:
		d.FieldStruct("gnu_hash", func(d *decode.D) {
			elfDecodeGNUHash(d, ec, ec.strTabMap[STRTAB_DYNSTR])
		})
	default:
		d.FieldRawLen("data", int64(size)*8)
	}
